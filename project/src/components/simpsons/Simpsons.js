...
export function Simpsons(){
    ...
    {name: 'Marge', surname: 'Simpson', age: 38,info:'Ма́рджори Жакли́н «Мардж» Си́мпсон (в девичестве Бувье́) (англ. Marjorie Jacqueline «Marge» Simpson) — постоянный персонаж мультипликационного сериала «Симпсоны», её озвучивает Джулия Кавнер. Обычно носит зелёное платье, красные балетки, на шее — ожерелье из искусственного жемчуга и ездит на оранжевом универсале.', photo: 'https://upload.wikimedia.org/wikipedia/ru/0/0b/Marge_Simpson.png'},
    ...
    return(
        ...
        <div className={'allSimpsons'}>

            {simpsons.map(value =>{
                return (<div className={'simpson'}>
                    <h1>{value.name}-{value.surname}-{value.age}</h1>
                    <div className={'info'}>
                        {value.info} <img src={value.photo} alt=""/>
                    </div>
                </div> )
            })}
        </div>
